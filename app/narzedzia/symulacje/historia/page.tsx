...
export default function HistoriaSymulacjiPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      // Uruchom polling jeśli są aktywne symulacje
      if (data.some((s) => ACTIVE.has(s.status))) {
        intervalRef.current = setInterval(async () => {
          const updated = await fetchSubmissions();
          // Zatrzymaj polling gdy żadna nie jest aktywna
    ...
    <div className="space-y-8">
      ...
      <form
        onSubmit={(e) => {
          ...
          if (val) window.location.href = `/narzedzia/symulacje/${val}`;
        }}
              ...
              <div key={s.case_id} className="bg-white dark:bg-[#1E232E]">
                {confirmDelete === s.case_id ? (
                  ...
                  <div className="flex items-center gap-4 px-4 py-4 group">
                    <Link href={`/narzedzia/symulacje/${s.case_id}`} className="flex items-center gap-4 flex-1 min-w-0">
