...
type Stats = {
  counts: { total: number; pending: number; running: number; done: number; failed: number; revenue: number; users: number };
  recent: Sim[];
};
...
function StatusCell({ caseId, initial, onChange }: { caseId: string; initial: string; onChange: (s: string) => void }) {
  ...
  return (
    <div ref={ref} className="relative">
      ...
      {open && (
        <div className="absolute top-full left-0 mt-1 z-50 w-36 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-[#1E232E] shadow-lg overflow-hidden">
...
export default function AdminPage() {
  ...
  const [tab, setTab] = useState<"pulpit" | "symulacje" | "uzytkownicy">("pulpit");
  ...
  if (access === "loading") {
    return <div className="py-20 text-center text-sm text-slate-400">Ładowanie…</div>;
  }
  if (access === "denied") {
    return (
      <div className="py-20 text-center">
        <p className="text-lg font-semibold text-slate-700 dark:text-slate-300">Brak dostępu</p>
        <p className="text-sm text-slate-400 mt-1">Ta strona jest dostępna tylko dla administratorów.</p>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-1 border-b border-slate-200 dark:border-slate-700">
        {(["pulpit", "symulacje", "uzytkownicy"] as const).map((t) => (
          <button key={t} onClick={() => setTab(t)}
            ...
            {t === "pulpit" ? "Pulpit" : t === "symulacje" ? "Symulacje" : "Użytkownicy"}
          </button>
        ))}
      </div>
                  ...
                  {stats!.recent.map(r => (
                    <tr key={r.case_id} className="bg-white dark:bg-[#1E232E] hover:bg-slate-50 dark:hover:bg-slate-800/40">
                      <td className="px-3 py-2">
                        <span className={`rounded-full px-2 py-0.5 text-[11px] font-semibold ${(S[r.status] ?? S.pending).cls}`}>
                          {(S[r.status] ?? S.pending).label}
                        </span>
                      </td>
                      ...
                      <td className="px-3 py-2 text-slate-400 whitespace-nowrap">{fmtDate(r.created_at)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
      ...
      {tab === "symulacje" && (
        <div className="space-y-4">
          ...
          {/* Table */}
          {simsLoading ? (
            <p className="text-sm text-slate-400 py-8 text-center">Ładowanie…</p>
          ) : (
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                  {sims.map(r => (
                    <tr key={r.case_id} className="bg-white dark:bg-[#1E232E] hover:bg-slate-50 dark:hover:bg-slate-800/40">
                      ...
                      <td className="px-3 py-2.5 text-slate-400 whitespace-nowrap">{fmtDate(r.created_at)}</td>
                    </tr>
                  ))}
                  {sims.length === 0 && (
                    <tr><td colSpan={8} className="px-3 py-10 text-center text-slate-400">Brak wyników</td></tr>
                  )}
                </tbody>
              </table>
            </div>
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-between text-xs text-slate-500">
              <span>{simsTotal} wyników, strona {simsPage} z {totalPages}</span>
      ...
      {tab === "uzytkownicy" && (
        <div className="space-y-4">
          <div className="flex gap-2">
            ...
            />
            <span className="text-xs text-slate-400 self-center">{filteredUsers.length} użytkowników</span>
          </div>
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                  {filteredUsers.map(u => (
                    <tr key={u.id} className="bg-white dark:bg-[#1E232E] hover:bg-slate-50 dark:hover:bg-slate-800/40">
                      ...
                      <td className="px-3 py-2.5 text-slate-400 whitespace-nowrap">{fmtDate(u.created_at)}</td>
                      <td className="px-3 py-2.5 text-slate-400 whitespace-nowrap">{u.last_sign_in_at ? fmtDate(u.last_sign_in_at) : "—"}</td>
