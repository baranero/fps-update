...
const CloudSpotlight = () => {
  return (
    <section className="relative overflow-hidden border-b border-cyan-500/10 bg-gradient-to-b from-[#0D1A2E] to-transparent py-20">
      {/* background glow */}
      <div
      ...
      <div className="container relative">
        <div className="grid grid-cols-1 gap-16 lg:grid-cols-2 lg:items-center">
          ...
          {/* Left: how it works */}
          <div>
            <div className="mb-4 inline-flex items-center gap-2 rounded-full border border-cyan-500/25 bg-cyan-500/10 px-3 py-1.5">
            ...
            <h2 className="mb-4 text-[clamp(26px,4vw,38px)] font-extrabold leading-[1.12] tracking-tight text-white text-wrap-balance">
              Obliczenia FDS{" "}
              <span className="text-cyan-400">bez własnej infrastruktury</span>
            </h2>
            ...
            <div className="mb-10 space-y-5">
              {steps.map(({ n, title, desc }) => (
                <div key={n} className="flex gap-4">
                  <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full border border-cyan-500/25 bg-cyan-500/10 font-mono text-[12px] font-bold text-cyan-400">
                    {n}
                  </div>
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-1 xl:grid-cols-2">
            {features.map(({ icon, title, desc }) => (
              <div
                key={title}
                className="rounded-xl border border-cyan-500/12 bg-cyan-500/[0.04] p-5 transition-colors hover:border-cyan-500/25"
              >
