...
export async function generateDOCX(data: CNBOPReportData, fileName = "Raport_CNBOP.docx") {
  ...
  if (!isMech) {
    ...
    const compRows: [string, string][] = [
      ["Konfiguracja napowietrzania", `${groupCount} ścieżka(i) równoległych, ${totalOpenings} otwór(y) łącznie`],
      ["Dobrana pow. efektywna napowietrzania (Aeff,komp)", `${data.compCalc.providedAeff.toFixed(2).replace(".", ",")} m²`],
    ];
